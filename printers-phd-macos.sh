...
# Printers:
#   - rsm-3n127-hp-color         / 3rd Floor / North / PhD Area
#   - rsm-3n127-xerox-bw-mac     / 3rd Floor / North / PhD Area
# Behavior:
...
ppd_for_queue() {
  # Decide PPD set based on queue name
  case "$1" in
    "$P1_NAME")
...
# Enable duplex & stapling without probing; ignore errors if option doesn't exist
enable_features_no_probe() {
  local printer="$1"
  # Duplex hardware presence (some PPDs use Option1/Duplexer etc.)
  ...
  # Stapler/finisher presence (effective with model PPDs)
  ...
  lpadmin -p "$printer" -o StapleUnit=Installed    >/devnull 2>&1 || true
}
...
add_printer() {
  ...
  # Description = printer name; require auth negotiation; optionally prefill username; hide driver deprecation msg
